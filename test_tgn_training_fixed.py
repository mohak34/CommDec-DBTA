...
Fixed version that properly handles model parameters.
"""
...
def test_tgn_training():
    ...
    try:
        ...
        # Test backward pass - but only optimize trainable parameters (exclude memory)
        loss_fn = nn.BCEWithLogitsLoss()
        loss = loss_fn(pos_prob, batch['label'])
        
        # Only get trainable parameters (excluding memory parameters)
        trainable_params = []
        for name, param in model.named_parameters():
            if param.requires_grad and 'memory' not in name and 'last_update' not in name:
