...
class DecayTGNMemoryModule(nn.Module):
    ...
    def __init__(self, num_nodes, memory_dim, message_dim, time_dim, 
                 decay_factor=0.1, adaptive_decay=False):
...
class DecayTemporalGraphNetwork(nn.Module):
    ...
    """
    def __init__(self, num_nodes, node_feat_dim, edge_feat_dim, memory_dim, 
                 time_dim, embedding_dim, decay_factor=0.1, adaptive_decay=False):
    ...
    def forward(self, src_ids, dst_ids, src_features, dst_features, 
        ...
        decay_analysis = {
            ...
            'avg_decay_score': (src_decay_info['decay_scores'].mean() + 
                              dst_decay_info['decay_scores'].mean()) / 2 if 
                              src_decay_info['decay_scores'] is not None else None
        }
    ...
    def get_decay_statistics(self, node_ids, current_time):
        ...
        """
        with torch.no_grad():
            memory, decay_scores = self.memory_module.get_memory(node_ids, 
                                                               torch.full_like(node_ids, current_time, dtype=torch.float))
            ...
            stats = {
                ...
                'avg_decay_score': decay_scores.mean() if decay_scores is not None else None,
                'min_decay_score': decay_scores.min() if decay_scores is not None else None,
                'max_decay_score': decay_scores.max() if decay_scores is not None else None,
...
def create_decay_tgn_model(num_nodes, node_feat_dim, edge_feat_dim, 
                          decay_factor=0.1, adaptive_decay=False, device='cpu'):
    """
...
def analyze_decay_effects(model, data_loader, decay_factors=[0.01, 0.05, 0.1, 0.2, 0.5]):
    """
        ...
        with torch.no_grad():
            for batch in data_loader:
                ...
                batch_results.append({
                    ...
                    'avg_decay_score': decay_analysis['avg_decay_score'].item() if 
                                     decay_analysis['avg_decay_score'] is not None else 0.0,
