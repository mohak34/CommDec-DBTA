...
class MessageFunction(nn.Module):
    ...
    def compute_message(self, raw_messages):
        ...
        raise NotImplementedError(
            "Message function must implement compute_message method"
        )
...
class MeanMessageAggregator(MessageAggregator):
    ...
    def aggregate(self, messages):
        ...
        """
        return (
            torch.mean(messages, dim=0)
            if len(messages) > 0
            else torch.zeros_like(messages[0])
        )
...
class LSTMAggregator(MessageAggregator):
    ...
    def __init__(self, message_dim, memory_dim):
        ...
        self.lstm = nn.LSTM(
            input_size=message_dim, hidden_size=memory_dim, batch_first=True
        )
...
class TGNMemoryModule(nn.Module):
    ...
    def reset_state(self):
        ...
        self.register_buffer(
            "memory", torch.zeros(self.num_nodes, self.memory_dim, device=device)
        )
    ...
    def update_state(self, source_nodes, destination_nodes, timestamps, edge_features):
        ...
        raw_messages = torch.cat(
            [source_memory, destination_memory, edge_features, source_time_features],
            dim=1,
        )
        ...
        for node in unique_nodes:
            ...
            if (
                node_idx in self.messages_buffer
                and len(self.messages_buffer[node_idx]) > 0
            ):
                ...
                updated_memory = self.memory_updater(
                    aggregated_message.unsqueeze(0), current_memory
                )

                # Update memory and last update time
                self.memory[node] = updated_memory.squeeze(0)
                self.last_update[node] = timestamps[
                    0
                ]  # Using the batch timestamp for simplicity
...
class TemporalAttentionLayer(nn.Module):
    ...
    def forward(
        ...
        query_raw = torch.cat(
            [node_features, node_time_features], dim=1
        )  # [batch_size, query_dim]
        query = self.query_projection(query_raw).unsqueeze(
            1
        ...
        key_raw = torch.cat(
            [
                neighbor_features.view(
                    batch_size * n_neighbors, -1
                ),  # Flatten neighbor features
                neighbor_time_features.view(
                    batch_size * n_neighbors, -1
                ),  # Flatten neighbor time features
                edge_features.view(
                    batch_size * n_neighbors, -1
            ...
            batch_size, n_neighbors, -1
        )  # [batch_size, n_neighbors, key_dim]
        ...
        value = self.value_projection(
            key_raw
        )  # [batch_size, n_neighbors, attention_dim]
...
class TemporalGraphNetwork(nn.Module):
    ...
    def __init__(
        ...
        if (
            self.memory_dim != self.embedding_dim and n_layers == 0
        ):  # only add if n_layers is 0 and dims differ
            self.final_embedding_projection = nn.Linear(
                self.memory_dim, self.embedding_dim
            )
        elif (
            n_layers > 0 and self.memory_dim != self.embedding_dim
        ):  # if layers exist, intermediate is memory_dim
    ...
    def compute_temporal_embeddings(
        ...
        if self.use_memory:
            ...
            node_memory = torch.zeros(
                batch_size, self.memory_dim, device=node_features.device
            )
        ...
        if neighbor_features is not None:
            neighbor_embeddings = self.node_embedding(
                neighbor_features
            ...
            neighbor_embeddings = torch.zeros(
                batch_size, n_neighbors, self.memory_dim, device=node_features.device
            )
        ...
        if neighbor_times is not None:
            ...
            neighbor_time_features = self.time_encoder(neighbor_times.view(-1, 1)).view(
                batch_size, -1, self.time_dim
            )
        else:
            # If no neighbor times, use zeros
            n_neighbors = neighbor_ids.size(1)
            neighbor_time_features = torch.zeros(
                batch_size, n_neighbors, self.time_dim, device=node_features.device
            )
    ...
    def forward(
        ...
        timestamps,
        edge_features,
        src_neighbor_ids=None,
        dst_neighbor_ids=None,
        src_neighbor_times=None,
        dst_neighbor_times=None,
        src_neighbor_features=None,
        dst_neighbor_features=None,
        src_edge_features=None,
        dst_edge_features=None,
    ):
        """
